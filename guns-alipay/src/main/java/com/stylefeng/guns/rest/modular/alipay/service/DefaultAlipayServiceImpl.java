...
public class DefaultAlipayServiceImpl implements AlipayServiceApi {
    ...
    public AlipayInfoVO getQRcode(String orderId) {
        ...
        if (StringUtils.isEmpty(filePath) || filePath.trim().length() == 0) {
            return null;
        } else {
    ...
    public AlipayRequestVO getOrderStatus(String orderId) {
        ...
        if (tradeQuery(orderId)) {
            ...
            alipayRequestVO.setOrderStatus(1);
            alipayRequestVO.setOrderMsg("支付成功");
    ...
    private String tradePrecreate(String orderId) {
        ...
        String operatorId = "wangwei";

        // (必填) 商户门店编号，通过门店号和商家后台可以配置精准到门店的折扣信息，详询支付宝技术支持
        String storeId = "wangweistore";
